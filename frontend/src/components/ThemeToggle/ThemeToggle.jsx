...
export default function ThemeToggle() {
  ...
  return <div className="theme-switcher" aria-label="Choose interface colour">
    ...
    <span className="theme-current" style={{ "--swatch": theme === "custom" ? customColor : palettes.find((palette) => palette.id === theme)?.color }} aria-hidden="true" />
    <div className="theme-picker" role="dialog" aria-label="Colour picker">
      ...
      <div className="theme-swatches">{palettes.map((palette) => <button key={palette.id} type="button" title={palette.label} aria-label={`Use ${palette.label} theme`} aria-pressed={theme === palette.id} className={`theme-swatch ${theme === palette.id ? "is-active" : ""}`} style={{ "--swatch": palette.color }} onClick={() => setTheme(palette.id)} />)}</div>

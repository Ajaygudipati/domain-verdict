...
import { Activity, AlertTriangle, ArrowLeft, CheckCircle2, ChevronDown, ClipboardCheck, FileWarning, Globe2, LockKeyhole, MailCheck, Network, Radar, ScanSearch, Server, ShieldCheck } from "lucide-react";
...
function DetailSection({ icon: Icon, title, status, children }) {
  ...
  return <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
    <button onClick={() => setOpen(!open)} className="flex w-full items-center justify-between gap-4 p-5 text-left hover:bg-slate-50">
      <div className="flex items-center gap-3"><div className="flex h-10 w-10 items-center justify-center rounded-xl bg-slate-100 text-slate-700"><Icon size={19} /></div><div><h2 className="font-semibold text-slate-950">{title}</h2><p className="text-sm text-slate-500">View scan evidence</p></div></div>
      <div className="flex items-center gap-3"><StatusBadge value={status} /><ChevronDown size={18} className={`text-slate-400 transition ${open ? "rotate-180" : ""}`} /></div>
    </button>
...
function Value({ label, value }) {
  return <div className="rounded-xl bg-slate-50 p-3"><p className="text-xs font-semibold uppercase tracking-wider text-slate-400">{label}</p><p className="mt-1 break-all text-sm font-medium text-slate-800">{value || "Not available"}</p></div>;
}
  ...
  return <MainLayout>
    <div id="report" className="min-h-screen bg-[#f8fafc] pb-16">
      <div className="mx-auto max-w-7xl px-5 pt-8 sm:px-8">
        ...
        <section className="mt-10"><div className="flex items-end justify-between gap-4"><div><p className="text-sm font-bold uppercase tracking-[0.14em] text-slate-400">Security posture</p><h2 className="mt-1 text-2xl font-bold text-slate-950">Category scores</h2></div><p className="hidden text-sm text-slate-500 sm:block">Select a category to focus its evidence below.</p></div><div className="mt-5 grid gap-4 sm:grid-cols-2 xl:grid-cols-5">{categoryConfig.map((category) => <CategoryCard key={category.key} {...category} score={summary.category_scores?.[category.key] ?? 0} onClick={() => setActiveCategory(category.key)} />)}</div></section>
        ...
        <section className="mt-10"><p className="text-sm font-bold uppercase tracking-[0.14em] text-slate-400">Evidence</p><h2 className="mt-1 text-2xl font-bold text-slate-950">Detailed scan report</h2><div className="mt-5 grid gap-4 lg:grid-cols-2">
          <DetailSection icon={LockKeyhole} title="SSL certificate" status={analysis.ssl?.status}><div className="grid gap-3 sm:grid-cols-2"><Value label="Issued to" value={ssl.issued_to} /><Value label="Issuer" value={ssl.issued_by} /><Value label="Expires" value={ssl.valid_until} /><Value label="Days remaining" value={ssl.days_left ?? null} /></div></DetailSection>
          <DetailSection icon={Server} title="Domain ownership" status={analysis.whois?.status}><div className="grid gap-3 sm:grid-cols-2"><Value label="Registrar" value={whois.registrar} /><Value label="Created" value={whois.creation_date} /><Value label="Expires" value={whois.expiration_date} /><Value label="Nameservers" value={Array.isArray(whois.name_servers) ? whois.name_servers.join(", ") : whois.name_servers} /></div></DetailSection>
          <DetailSection icon={MailCheck} title="Email authentication" status={analysis.dmarc?.status}><div className="grid gap-3 sm:grid-cols-3"><Value label="SPF" value={analysis.spf?.data?.enabled ? "Configured" : "Missing"} /><Value label="DKIM" value={analysis.dkim?.data?.enabled ? `Configured (${analysis.dkim.data.selector})` : "Not detected"} /><Value label="DMARC policy" value={analysis.dmarc?.data?.policy} /></div></DetailSection>
          <DetailSection icon={Radar} title="Threat intelligence" status={analysis.virustotal?.status}><div className="grid gap-3 sm:grid-cols-2"><Value label="VirusTotal reputation" value={threat.reputation} /><Value label="Malicious detections" value={threat.malicious} /><Value label="Abuse confidence" value={analysis.abuseipdb?.data?.abuse_confidence_score != null ? `${analysis.abuseipdb.data.abuse_confidence_score}%` : null} /><Value label="Safe Browsing" value={analysis.google_safe_browsing?.data?.unsafe ? "Flagged" : "No flag returned"} /></div></DetailSection>
        ...
        {activeCategory && <p className="mt-5 text-center text-sm text-slate-500">Focused category: <span className="font-semibold text-slate-800">{categoryConfig.find((item) => item.key === activeCategory)?.name}</span></p>}
      </div>
    </div>
  </MainLayout>;
}

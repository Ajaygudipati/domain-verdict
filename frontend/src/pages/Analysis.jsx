...
import DomainAdvisor from "../components/dashboard/DomainAdvisor";
...
function DetailSection({ icon: Icon, title, status, children }) {
  return <section className="terminal-detail overflow-hidden rounded-2xl border">
    <div className="flex items-center justify-between gap-4 border-b border-white/10 p-5">
      <div className="flex items-center gap-3"><div className="terminal-detail-icon flex h-10 w-10 items-center justify-center rounded-xl"><Icon size={19} /></div><div><h2 className="font-semibold text-white">{title}</h2><p className="text-sm text-slate-500">Scan evidence</p></div></div>
...
function Value({ label, value, highlight = false, tone = "slate" }) {
  const tones = {
    slate: "border-white/10 bg-white/[.035]",
  ...
  return <div className={`rounded-xl border p-3 ${tones[tone]}`}><p className="text-xs font-semibold uppercase tracking-wider text-slate-500">{label}</p><p className={`mt-1 break-all text-sm ${highlight ? "font-bold text-white" : "font-medium text-slate-300"}`}>{displayValue}</p></div>;
}
...
export default function Analysis() {
  ...
  return <MainLayout hero>
    <div id="report" className="terminal-report min-h-screen pb-20 pt-24">
      <div className="mx-auto max-w-7xl px-5 pt-8 sm:px-8">
        <button onClick={() => navigate("/")} className="terminal-back inline-flex items-center gap-2 text-sm font-semibold transition"><ArrowLeft size={16} /> [ New scan ]</button>
        <header className="terminal-report-hero mt-5 overflow-hidden rounded-3xl border px-6 py-8 text-white sm:px-10">
          <div className="grid gap-8 lg:grid-cols-[1fr_auto] lg:items-center"><div><div className="flex flex-wrap items-center gap-3"><div className="terminal-globe flex h-11 w-11 items-center justify-center rounded-xl"><Globe2 size={21} /></div><StatusBadge value={summary.verdict} /></div><p className="mt-5 font-mono text-xs font-bold tracking-[.16em]" style={{ color: "var(--neon)" }}>$ REPORT / COMPLETE</p><h1 className="mt-2 break-all font-mono text-3xl font-bold tracking-tight sm:text-5xl">{scanInfo.domain}</h1><p className="mt-3 max-w-xl text-sm leading-6 text-slate-300">Security posture assessed across email, infrastructure, website configuration, and global threat-intelligence signals.</p><div className="mt-6 flex flex-wrap gap-5 text-sm text-slate-300"><span className="inline-flex items-center gap-2"><ClipboardCheck size={16} /> {formatTime}</span><span className="inline-flex items-center gap-2"><Activity size={16} /> Confidence: {summary.confidence}</span>{summary.scan_coverage && <span className="inline-flex items-center gap-2"><ScanSearch size={16} /> Sources: {summary.scan_coverage.completed_sources}/{summary.scan_coverage.total_sources}</span>}</div><div className="mt-6"><DomainAdvisor data={data} /></div></div><div className="terminal-score-wrap rounded-3xl p-3"><ScoreCard score={summary.overall_score} verdict={summary.verdict} /></div></div>
        </header>

        <section className="mt-10"><div className="flex items-end justify-between gap-4"><div><p className="text-sm font-bold uppercase tracking-[0.14em]" style={{ color: "var(--neon)" }}>Security posture</p><h2 className="mt-1 text-2xl font-bold text-white">Category scores</h2></div><p className="hidden text-sm text-slate-500 sm:block">Every score shows its earned points and category maximum.</p></div><div className="mt-5 grid gap-4 sm:grid-cols-2 xl:grid-cols-5">{categoryConfig.map((category) => <CategoryCard key={category.key} {...category} score={summary.category_scores?.[category.key] ?? 0} />)}</div></section>

        <section className="mt-10 grid gap-6 lg:grid-cols-[1.35fr_.65fr]">
          <div><div className="flex items-center gap-3"><AlertTriangle size={21} className={issues.length ? "text-amber-500" : "text-emerald-500"} /><h2 className="text-2xl font-bold text-white">{issues.length ? "Findings that need attention" : "No material findings"}</h2></div><div className="terminal-panel mt-5 rounded-2xl border p-2">{issues.length ? issues.map((issue, index) => <div key={`${issue}-${index}`} className="flex gap-3 rounded-xl p-4"><span className="mt-0.5 flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-amber-300/15 text-xs font-bold text-amber-300">{index + 1}</span><p className="text-sm leading-6 text-slate-300">{issue}</p></div>) : <div className="flex items-center gap-3 p-5 text-sm text-slate-300"><CheckCircle2 style={{ color: "var(--neon)" }} /> No scoring deductions were reported by the completed checks.</div>}</div></div>
          <aside className="terminal-panel rounded-2xl border p-6"><p className="text-sm font-bold uppercase tracking-[0.14em]" style={{ color: "var(--neon)" }}>Recommended action</p><h2 className="mt-2 text-xl font-bold text-white">Improve the weak signals</h2><div className="mt-5 space-y-4">{recommendations.length ? recommendations.slice(0, 4).map((item) => <div key={item} className="border-l-2 pl-3 text-sm leading-6 text-slate-300" style={{ borderColor: "var(--neon)" }}>{item}</div>) : <p className="text-sm leading-6 text-slate-500">The scan did not return any specific configuration recommendations.</p>}</div></aside>
        </section>

        <section className="mt-10"><p className="text-sm font-bold uppercase tracking-[0.14em]" style={{ color: "var(--neon)" }}>Evidence</p><h2 className="mt-1 text-2xl font-bold text-white">Detailed scan report</h2><p className="mt-2 text-sm text-slate-500">All evidence is visible by default, with the most useful signals highlighted.</p><div className="mt-5 grid gap-4 lg:grid-cols-2">
          <DetailSection icon={LockKeyhole} title="SSL certificate" status={analysis.ssl?.status}><div className="mb-4 flex items-center gap-2 text-sm font-semibold" style={{ color: "var(--neon)" }}><BadgeCheck size={17} /> HTTPS certificate details</div><div className="grid gap-3 sm:grid-cols-2"><Value label="Issued to" value={ssl.issued_to} highlight /><Value label="Issuer" value={ssl.issued_by} /><Value label="Expires" value={ssl.valid_until} highlight tone={ssl.days_left < 14 ? "amber" : "green"} /><Value label="Days remaining" value={ssl.days_left ?? null} highlight tone={ssl.days_left < 14 ? "amber" : "green"} /></div></DetailSection>
          <DetailSection icon={Server} title="Domain ownership" status={analysis.whois?.status}><div className="mb-4 flex items-center gap-2 text-sm font-semibold text-slate-300"><CalendarDays size={17} /> Key registration details</div><div className="grid gap-3 sm:grid-cols-2"><Value label="Registrar" value={whois.registrar} highlight /><Value label="Created" value={whois.creation_date} highlight tone="green" /><Value label="Expires" value={whois.expiration_date} highlight /><Value label="Nameservers" value={Array.isArray(whois.name_servers) ? whois.name_servers.join(", ") : whois.name_servers} /></div></DetailSection>

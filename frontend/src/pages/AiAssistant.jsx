import { Bot, CalendarDays, CheckCircle2, Download, FileDown, Globe2, LoaderCircle, Network, RotateCcw, Send, Share2, ShieldAlert, ShieldCheck, Sparkles, Trash2 } from "lucide-react";
...
export default function AiAssistant() {
  ...
  async function shareReport() {
    if (!report?.scan_info?.scan_id || !token) return;
    const response = await fetch(`${API_URL}/reports/${report.scan_info.scan_id}/share?token=${encodeURIComponent(token)}`, { method: "POST" });
    const data = response.ok ? await response.json() : null;
    if (data?.token) { await navigator.clipboard.writeText(`${window.location.origin}/shared/${data.token}`); window.alert("Share link copied to your clipboard."); }
  }
